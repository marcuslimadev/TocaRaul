...
if($method==='GET'&&$path==='/api/device/state'){
 ...
 if($state===null){
  ...
  $state=['connection'=>'ONLINE',
   ...
   'nowPlaying'=>$playing?['id'=>(string)$playing['id'],'providerId'=>$playing['providerId'],'title'=>$playing['title'],'artist'=>$playing['artist'],'message'=>((int)($playing['isReplay']??0)===1?null:$playing['message']),'visitorName'=>((int)($playing['isReplay']??0)===1?null:$playing['visitorName']),'tableCode'=>$playing['tableCode']]:null,
