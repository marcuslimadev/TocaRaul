...
if($method==='POST'&&$path==='/api/player/random'){$dev=device();$vid=(int)$dev['venueId'];$d=db();$d->beginTransaction();try{$q=$d->prepare("SELECT * FROM songRequests WHERE venueId=? AND status='PLAYED' ORDER BY RAND() LIMIT 1 FOR UPDATE");$q->execute([$vid]);$r=$q->fetch();if($r){$d->prepare("UPDATE songRequests SET status='PLAYING',isReplay=1 WHERE id=?")->execute([(int)$r['id']]);
 // Repeticao do historico: a dedicatoria era pra quem pediu na hora, nao faz
 // sentido mostrar de novo pra quem esta vendo a TV agora.
 $track=['id'=>(string)$r['id'],'providerId'=>$r['providerId'],'title'=>$r['title'],'artist'=>$r['artist'],'message'=>null,'tableCode'=>$r['tableCode'],'visitorName'=>null,'source'=>'RANDOM_HISTORY'];}else{$q=$d->prepare("SELECT * FROM barPlaylist WHERE venueId=? AND status='PLAYED' ORDER BY RAND() LIMIT 1 FOR UPDATE");$q->execute([$vid]);$b=$q->fetch();if($b){$d->prepare("UPDATE barPlaylist SET status='PLAYING' WHERE id=?")->execute([(int)$b['id']]);$track=['id'=>(string)(-(int)$b['id']),'providerId'=>$b['providerId'],'title'=>$b['title'],'artist'=>$b['artist'],'message'=>null,'tableCode'=>null,'source'=>'RANDOM_HISTORY'];}else$track=null;}$d->commit();}catch(Throwable$e){$d->rollBack();throw$e;}invalidate_venue_state($vid);out(['track'=>$track]);}
