...
if(preg_match('~^/assets/([A-Za-z0-9._-]+)$~',$path,$m)&&is_file($root.'/assets/'.$m[1])){
 $types=['js'=>'application/javascript','png'=>'image/png','svg'=>'image/svg+xml','jpg'=>'image/jpeg','json'=>'application/json','webmanifest'=>'application/manifest+json'];
...
elseif($path==='/player'||$path==='/tela')require $root.'/player.php';
elseif($path==='/tv'){header('Location: /player',true,302);return;}
