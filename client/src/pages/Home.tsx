...
function ClientView() {
  ...
  const handleContinueToPix = async () => { if (!modalSong) return; const created = await mockCreate.mutateAsync({ title: modalSong.title, artist: modalSong.artist }); setMockPayment({ requestId: created.requestId, externalId: created.externalId }); setSelected(null); setOrdered(true); };
  const handleMockConfirm = async () => { if (!mockPayment) { setPaid(true); return; } await mockConfirm.mutateAsync(mockPayment); setPaid(true); };
