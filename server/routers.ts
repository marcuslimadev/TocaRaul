...
export const appRouter = router({
  ...
  requests: router({
    mockCreate: publicProcedure.input(z.object({ title: z.string().min(1), artist: z.string().min(1) })).mutation(({ input }) => { const requestId = nextMockRequestId++; const externalId = `mock_${requestId}`; mockRequests.set(requestId, { requestId, externalId, title: input.title, artist: input.artist, paymentStatus: "PENDING", requestStatus: "AWAITING_PAYMENT" }); return { requestId, externalId, pixCopyPaste: `000201MOCKTOCARAUL${requestId}`, status: "AWAITING_PAYMENT" as const }; }),
    create: publicProcedure.input(z.object({ venueId: z.number().int().positive(), visitorName: z.string().min(1).max(80), tableCode: z.string().max(12).optional(), providerId: z.string().min(1).max(160), title: z.string().min(1).max(180), artist: z.string().min(1).max(180), message: z.string().max(180).optional(), amountCents: z.number().int().nonnegative() })).mutation(async ({ input }) => { const requestId = await createPendingRequest(input); const payment = await new MockPaymentProvider().createPayment({ requestId, amountCents: input.amountCents, description: `${input.title} — ${input.artist}` }); const paymentId = await createPendingPayment({ requestId, amountCents: input.amountCents, externalId: payment.externalId, pixCopyPaste: payment.pixCopyPaste }); return { requestId, paymentId, externalId: payment.externalId, pixCopyPaste: payment.pixCopyPaste, status: "AWAITING_PAYMENT" as const }; }),
  ...
  payments: router({
    mockStatus: publicProcedure.input(z.object({ requestId: z.number().int().positive() })).query(async ({ input }) => { const inMemory = mockRequests.get(input.requestId); if (inMemory) return { paymentStatus: inMemory.paymentStatus, requestStatus: inMemory.requestStatus, externalId: inMemory.externalId, pixCopyPaste: `000201MOCKTOCARAUL${inMemory.requestId}` }; const payment = await getPaymentByRequestId(input.requestId); const request = await getRequestById(input.requestId); if (!payment || !request) return null; const providerStatus = payment.externalId ? await new MockPaymentProvider().getPayment(payment.externalId) : "PENDING"; return { paymentStatus: providerStatus, requestStatus: request.status, externalId: payment.externalId, pixCopyPaste: payment.pixCopyPaste }; }),
    mockConfirm: publicProcedure.input(z.object({ requestId: z.number().int().positive(), externalId: z.string().regex(/^mock_/) })).mutation(async ({ input }) => { const inMemory = mockRequests.get(input.requestId); if (inMemory && inMemory.externalId === input.externalId) { inMemory.paymentStatus = "APPROVED"; inMemory.requestStatus = "QUEUED"; return { success: true as const, status: "QUEUED" as const }; } const updated = await markRequestQueuedAfterApprovedPayment(input.requestId, input.externalId); if (!updated) throw new TRPCError({ code: "CONFLICT", message: "Mock payment is not pending or request is already queued" }); return { success: true as const, status: "QUEUED" as const }; }),
