...
import { devices, InsertUser, payments, songRequests, users, venues, venueTables, venueBalanceEntries, venuePayouts } from "../drizzle/schema";
...
export async function recordVenueEarning(requestId: number, paymentExternalId: string) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const request = await getRequestById(requestId); const venue = request ? await getVenueById(request.venueId) : undefined; const payment = await getPaymentByRequestId(requestId); if (!request || !venue || !payment) return false; const externalReference = `earning_${paymentExternalId}`; const existing = await db.select().from(venueBalanceEntries).where(eq(venueBalanceEntries.externalReference, externalReference)).limit(1); if (existing[0]) return true; const barShare = Math.round(payment.amountCents * (venue.splitBarPercent ?? 70) / 100); await db.insert(venueBalanceEntries).values({ venueId: venue.id, requestId, type: "EARNING", amountCents: barShare, externalReference, status: "AVAILABLE" }); return true; }
export async function listVenueBalance(venueId: number) { const db = await getDb(); if (!db) return { availableCents: 0, pendingCents: 0, paidOutCents: 0, entries: [] }; const entries = await db.select().from(venueBalanceEntries).where(eq(venueBalanceEntries.venueId, venueId)); return { availableCents: entries.filter(entry => entry.status === "AVAILABLE").reduce((sum, entry) => sum + entry.amountCents, 0), pendingCents: entries.filter(entry => entry.status === "PENDING").reduce((sum, entry) => sum + entry.amountCents, 0), paidOutCents: entries.filter(entry => entry.type === "PAYOUT" && entry.status === "DONE").reduce((sum, entry) => sum + Math.abs(entry.amountCents), 0), entries }; }
export async function createVenuePayout(input: typeof venuePayouts.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const existing = await db.select().from(venuePayouts).where(eq(venuePayouts.externalReference, input.externalReference)).limit(1); if (existing[0]) return existing[0]; const result = await db.insert(venuePayouts).values(input); const rows = await db.select().from(venuePayouts).where(eq(venuePayouts.id, Number(result[0].insertId))).limit(1); return rows[0]; }
export async function updateVenuePayout(id: number, patch: Partial<typeof venuePayouts.$inferInsert>) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); await db.update(venuePayouts).set(patch).where(eq(venuePayouts.id, id)); const rows = await db.select().from(venuePayouts).where(eq(venuePayouts.id, id)).limit(1); return rows[0]; }
export async function createVenuePayoutEntry(input: typeof venueBalanceEntries.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const existing = await db.select().from(venueBalanceEntries).where(eq(venueBalanceEntries.externalReference, input.externalReference)).limit(1); if (existing[0]) return existing[0]; const result = await db.insert(venueBalanceEntries).values(input); const rows = await db.select().from(venueBalanceEntries).where(eq(venueBalanceEntries.id, Number(result[0].insertId))).limit(1); return rows[0]; }
