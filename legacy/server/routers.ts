...
import { createPendingPayment, createPendingRequest, getPaymentByRequestId, getRequestById, getVenueByCode, getVenueById, listQueue, listVenueBalance, markRequestQueuedAfterApprovedPayment, updateRequestStatus, updateVenuePagarmeOnboarding, updateVenuePricing } from "./db";
...
export const appRouter = router({
  ...
  venue: router({
    ...
    balance: protectedProcedure.input(z.object({ venueId: z.number().int().positive() })).query(async ({ input, ctx }) => {
      const venue = await getVenueById(input.venueId);
      if (!venue || venue.ownerId !== ctx.user.id) throw new TRPCError({ code: "FORBIDDEN" });
      return listVenueBalance(input.venueId);
    }),
    runDailyPayout: protectedProcedure.input(z.object({ venueId: z.number().int().positive(), date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional() })).mutation(({ input }) => runVenueDailyPayout(input.venueId, input.date)),
  ...
  requests: router({
    ...
    create: publicProcedure.input(z.object({ venueId: z.number().int().positive(), visitorName: z.string().min(1).max(80), tableCode: z.string().max(12).optional(), providerId: z.string().min(1).max(160), title: z.string().min(1).max(180), artist: z.string().min(1).max(180), message: z.string().max(180).optional(), amountCents: z.number().int().nonnegative(), payerEmail: z.string().email().optional() })).mutation(async ({ input }) => { const venue = await getVenueById(input.venueId); if (!venue) throw new TRPCError({ code: "NOT_FOUND", message: "Venue not found" }); const requestId = await createPendingRequest(input); const provider = activePaymentProvider(); const configuredProvider = process.env.PAYMENT_PROVIDER?.toLowerCase(); const paymentProviderName = configuredProvider === "mercadopago" || configuredProvider === "pagarme" || configuredProvider === "asaas" ? configuredProvider : "mock"; const payment = await provider.createPayment({ requestId, amountCents: input.amountCents, description: `${input.title} — ${input.artist}`, venueRecipientId: venue.pagarmeRecipientId ?? undefined, splitBarPercent: venue.splitBarPercent ?? 70, mercadoPagoAccessToken: venue.mercadoPagoAccessToken ?? undefined, payerEmail: input.payerEmail }); const paymentId = await createPendingPayment({ requestId, provider: paymentProviderName, amountCents: input.amountCents, externalId: payment.externalId, pixCopyPaste: payment.pixCopyPaste }); return { requestId, paymentId, externalId: payment.externalId, pixCopyPaste: payment.pixCopyPaste, status: "AWAITING_PAYMENT" as const }; }),
