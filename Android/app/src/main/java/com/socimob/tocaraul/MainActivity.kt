...
class MainActivity:ComponentActivity(){
 private var pendingRequestId:String?=null;private var pendingDeviceToken:String?=null;private var queue:RemotePlayerQueueRepository?=null;private lateinit var api:TocaRaulApiClient
 ...
 override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);WindowCompat.setDecorFitsSystemWindows(window,false);api=TocaRaulApiClient(BuildConfig.TOCARAUL_API_BASE_URL);val devices=RemoteDeviceRepository(api);queue=RemotePlayerQueueRepository(api);setContent{TocaRaulTheme{var state by remember{mutableStateOf(connecting())};LaunchedEffect(Unit){val store=DeviceIdentityStore(applicationContext);var session:DeviceSession?=store.loadSession();var retry=1000L;while(true){try{if(session==null){state=connecting();session=devices.createSession("TV Principal");store.saveSession(requireNotNull(session))};val current=requireNotNull(session);val next=devices.state(current.deviceToken);retry=1000;if(next.connection==ConnectionState.WaitingActivation){state=activation(current);delay(3000);continue};if(next.connection==ConnectionState.Online){store.markActivated();state=next;try{devices.heartbeat(current.deviceToken)}catch(_:Throwable){};if(pendingRequestId==null){try{val claimed=queue?.claim(current.deviceToken);val video=claimed?.track?.providerId?.removePrefix("youtube:");if(claimed!=null&&video!=null&&video.matches(Regex("^[A-Za-z0-9_-]{11}$"))){pendingRequestId=claimed.track.id;pendingDeviceToken=current.deviceToken;playback.launch(Intent(this@MainActivity,PlaybackActivity::class.java).putExtra(PlaybackActivity.EXTRA_VIDEO_ID,video).putExtra(PlaybackActivity.EXTRA_DEVICE_TOKEN,current.deviceToken).putExtra(PlaybackActivity.EXTRA_COMMAND_URL,api.playerCommandUrl()))}else if(claimed!=null)queue?.complete(current.deviceToken,claimed.track.id,"SKIPPED")}catch(_:Throwable){}};delay(2000);continue};state=next;delay(3000)}catch(_:Throwable){state=reconnecting();delay(retry);retry=(retry*2).coerceAtMost(30000);session=store.loadSession()}}};JukeboxScreen(state)}}}
