...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val api = TocaRaulApiClient(BuildConfig.TOCARAUL_API_BASE_URL)
        val devices = RemoteDeviceRepository(api)
        queueRepository = RemotePlayerQueueRepository(api)
        setContent {
            TocaRaulTheme {
                ...
                LaunchedEffect(Unit) {
                    ...
                    while (true) {
                        try {
                            if (session == null) { state = connectingState(); session = devices.createSession("TV Principal"); identityStore.saveSession(session) }
                            val current = session
                            val next = devices.state(current.deviceToken)
                            retryDelayMs = 1_000L
                            if (next.connection == ConnectionState.WaitingActivation) { state = activationState(current); delay(3_000); continue }
                            if (next.connection == ConnectionState.Online) {
                                identityStore.markActivated(); state = next
                                try { devices.heartbeat(current.deviceToken) } catch (_: Throwable) {}
                                if (pendingRequestId == null) {
                                    try {
                                        val claimed = queueRepository?.claim(current.deviceToken)
                                        val videoId = claimed?.track?.providerId?.removePrefix("youtube:")
                                        if (claimed != null && videoId != null && videoId.matches(Regex("^[A-Za-z0-9_-]{11}$"))) {
                                            pendingRequestId = claimed.track.id; pendingDeviceToken = current.deviceToken
                                            playbackLauncher.launch(Intent(this@MainActivity, PlaybackActivity::class.java).putExtra(PlaybackActivity.EXTRA_VIDEO_ID, videoId))
                                        } else if (claimed != null) {
                                            queueRepository?.complete(current.deviceToken, claimed.track.id, "SKIPPED")
                                        }
                                    } catch (_: Throwable) {}
                                }
                                delay(3_000); continue
                            }
                            state = next; delay(3_000)
                        } catch (_: Throwable) { state = reconnectingState(); delay(retryDelayMs); retryDelayMs = (retryDelayMs * 2).coerceAtMost(30_000L); session = identityStore.loadSession() }
                    }
                }
